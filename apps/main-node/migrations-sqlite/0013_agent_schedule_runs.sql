...
-- self-host Node control-plane DB (sqlite dialect). agent_schedules only
-- tracks the LATEST firing (last_run_*); this table gives every firing
-- (ok / error / skipped_concurrency) its own durable row so run history can
-- be paged. `summary` is a nullable slot for a follow-up (see AGENTS.md).
