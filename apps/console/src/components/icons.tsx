...
/**
 * AnyRouter brand mark — the interlocking "A" router glyph from
 * anyrouter.dev/brand/anyrouter-logo.svg. A fill-rendered mark that uses
 * `currentColor` so it inherits the parent text color (theme-aware without
 * extra CSS). The original SVG's viewBox is preserved — the logo is a
 * detailed glyph that needs room to read, so it's sized larger than the
 * 16px lucide icons via className (e.g. `w-14 h-14` on the connect dialog).
 */
export function AnyRouterIcon({ className }: IconProps) {
  return (
    <svg
      className={className ?? "w-4 h-4"}
