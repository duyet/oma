...
function findByPath(
  ...
  const direct = byId.get(`path:${path}`);
...
export function computeSessionArtifacts(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    switch (e.type) {
      ...
      case "agent.output_declared": {
        const path = asString(e.path);
        const id = asString(e.tool_use_id);
        if (!path || !id) break;
        const name = basename(path);
        const mediaType = asString(e.media_type);
        const description = asString(e.description);
        const sizeBytes =
          typeof e.size_bytes === "number" && Number.isFinite(e.size_bytes)
            ? e.size_bytes
            : undefined;
        const data = asString(e.data);
        const src = data && mediaType ? `data:${mediaType};base64,${data}` : undefined;
        const prev = findByPath(byId, path);
        const artifact: SessionArtifact = {
          id: prev?.id ?? `declared:${id}`,
          name,
          path,
          source: prev?.source ?? "tool_output",
          kind: kindOf(name, mediaType ?? prev?.mediaType),
          mediaType: mediaType ?? prev?.mediaType,
          extension: extensionOf(name),
          sizeBytes: sizeBytes ?? prev?.sizeBytes,
          ts: ts ?? prev?.ts,
          toolName: prev?.toolName ?? "output_file",
          isError: prev?.isError,
          declared: true,
          description,
          text: prev?.text,
          preview:
            prev?.preview && prev.preview.kind !== "none"
              ? prev.preview
              : previewFor({ name, mediaType, src, text: prev?.text }),
          downloadHref: prev?.downloadHref ?? src,
        };
        if (prev && prev.id !== artifact.id) byId.delete(prev.id);
        upsert(byId, artifact);
        break;
      }
      default:
        break;
