...
describe("computeSessionArtifacts", () => {
  ...
  it("badges agent.output_declared deliverables and merges by path", () => {
    const artifacts = computeSessionArtifacts([
      ev({
        type: "agent.tool_use",
        name: "output_file",
        tool_use_id: "toolu_1",
        ts: "2026-08-03T14:31:00Z",
        input: { path: "/workspace/output/report.pdf" },
      }),
      ev({
    ...
    const report = artifacts.find((a) => a.name === "report.pdf");
    expect(report?.declared).toBe(true);
    expect(report?.description).toBe("Weekly metrics digest");
    expect(report?.sizeBytes).toBe(89432);
    expect(report?.kind).toBe("pdf");
    const summary = artifacts.find((a) => a.name === "summary.md");
