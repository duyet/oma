...
2. Fill in:
   ...
   - **Model** — pick from your Model Cards. A platform-default Claude handle (`claude-sonnet-4-6`) is listed when the tenant has no cards of its own (deployment `ANTHROPIC_API_KEY` or `ANYROUTER_API_KEY` fallback). Add a Model Card under **Settings → Model Cards** to bind a different provider or key.
