...
# API_KEY is prefilled as dev-test-key-change-me (same as .dev.vars.example).
# A provider key is required for a successful turn — set ANTHROPIC_API_KEY
# or add a Model Card in the Console.
$EDITOR .env
...
Drive the harness end-to-end (uses `API_KEY` from `.env`; sessions need an environment, same as `oma sessions create --agent … --env …`):
...
| **AUTH_DISABLED=1** | "I'm the only user, skip auth entirely" | `echo "AUTH_DISABLED=1" >> .env`, restart. Every request becomes `tenant_id=default`, `/auth-info` returns `providers: []`, and the Console **skips login**. Do not use in production. |
...
- **`AUTH_DISABLED=1` belongs in `.env`, not the shell.** Compose
  ...
  dir — exporting it only in your shell is silently ignored. With the
  flag on, the Console skips login (`/auth-info` returns `providers: []`).
