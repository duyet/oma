...
| `ANYROUTER_API_KEY` | agent | Static fallback provider (routes through [AnyRouter](https://anyrouter.dev), an OpenAI-compatible gateway) used only when an agent's model matches no Model Card **and** `ANTHROPIC_API_KEY` is unset. Bare `claude-*` handles are rewritten to `anthropic/claude-*` on this path (the seeded General agent's `claude-sonnet-4-6` is valid). Do not send the dotted alias `anthropic/claude-sonnet-4.6` — that catalog entry is BYOK-only. |
