...
1. **Install Node 22+ and pnpm 10+.** A Cloudflare account and `wrangler login` are **not** required for `pnpm dev` (local simulators).
...
4. **Run.** Pick what you need:
   ...
   ```bash
   # API + Console on http://localhost:8787 (most common dev loop)
   # predev builds apps/console/dist and copies .dev.vars next to worker configs
   pnpm dev
   ...
   # Console Vite HMR (optional; ASSETS already serves the SPA on :8787)
