...
function StatusDot({ status, dashed }: { status?: FitCard["status"]; dashed?: boolean }) {
  ...
  return <span className={cn("shrink-0 w-1.5 h-1.5 rounded-full", DOT_CLASS[resolved])} aria-hidden="true" />;
}
  ...
  return (
    <div className="mt-1.5 flex flex-wrap gap-1">
      {shown.map((n) => (
        <span
          key={n}
          className="inline-flex max-w-[9.5rem] truncate rounded-md border border-border bg-bg-surface/50 px-1.5 py-0.5 text-[11px] leading-tight text-fg-muted"
        >
          {n}
        </span>
      ))}
      {rest > 0 && (
        <span className="inline-flex rounded-md px-1 py-0.5 text-[11px] leading-tight text-fg-subtle">
          +{rest}
        </span>
      )}
    </div>
    ...
    <div className="mt-2 flex items-center -space-x-1.5">
      {norm.slice(0, 5).map((m) => (
        <span
          ...
          className={cn(
            "flex size-6 items-center justify-center rounded-full border border-border bg-bg ring-2 ring-bg-surface",
      ...
      {norm.length > 5 && (
        <span className="flex size-6 items-center justify-center rounded-full border border-border bg-bg ring-2 ring-bg-surface text-[10px] font-medium text-fg-muted">
          +{norm.length - 5}
        </span>
      )}
    </div>
    ...
    >
      <div className="flex items-center gap-2">
        {card.icon && (
          <span
            className={cn(
              "shrink-0 text-fg-subtle transition-colors group-hover:text-fg-muted",
              card.hero && "text-brand group-hover:text-brand",
            )}
            aria-hidden="true"
          >
            {card.icon}
