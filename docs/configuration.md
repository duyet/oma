...
| `AUTH_DISABLED` | No | Set to `1` for a **single-user local trial**. Every request becomes `tenant_id="default"` (no `x-api-key`), `/auth-info` returns `providers: []`, and the Console skips login. `/auth/*` returns 410 while auth is off so leftover signup UI cannot 404. **Self-host Node and CF wrangler local.** Do not use in production. |
| `CONSOLE_DIR` | No | Self-host Node: path to the built Console SPA (`index.html`). Docker images set this. Local `pnpm --filter @duyet/oma-main-node start` also auto-detects `apps/console/dist`. Unset + no dist → API-only on `:8787` (use `pnpm dev:console` on `:5173`). |
